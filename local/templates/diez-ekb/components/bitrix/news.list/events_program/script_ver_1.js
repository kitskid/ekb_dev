...
function getSelectedCity() {
    const cityDropdown = document.querySelector('[data-filter="city"]').closest('.filter-dropdown');
    const selectedOption = cityDropdown.querySelector('.filter-option.selected');
    return selectedOption ? selectedOption.dataset.value : '';
}

// Функция для получения выбранного месяца
function getSelectedMonth() {
    const monthDropdown = document.querySelector('[data-filter="month"]').closest('.filter-dropdown');
    const selectedOption = monthDropdown.querySelector('.filter-option.selected');
    return selectedOption ? selectedOption.dataset.value : '';
}
...
function updateResetButton(filterContainer, activeFilters) {
    ...
    const hasActiveFilters = activeFilters.length > 0 || getSelectedCity() || getSelectedMonth();
...
function loadFilteredEvents(tags, containerId, city = '', month = '') {
    ...
    // Если параметры не переданы, получаем их из интерфейса
    if (!city) city = getSelectedCity();
