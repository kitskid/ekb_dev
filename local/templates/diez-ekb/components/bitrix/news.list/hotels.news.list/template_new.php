...
<section class="section section--gradient pt-130 ptm-64 pb-125 pbm-30">
    <div class="container">
        ...
        <div class="hotels">
            <div class="filter" id="<?=$filterContainerId?>">
            ...
            <?php if ($arResult["SHOW_MORE_BUTTON"]): ?>
                <button class="button button--down js-load-more" data-page="1" data-container="<?=$containerId?>" data-max-page="<?=$arResult['NAV_RESULT']->NavPageCount?>">
    ...
    <section class="section section--logo-bg section--logo-bg-top pt-130 ptm-55 pb-130 pbm-80">
        <div class="container">
            ...
            <div class="map">
                <div id="hotels-map" style="width:100%; height:450px;"></div>
            </div>
        </div>
    </section>
...
<?php else: ?>
    <?php if ($arResult["SHOW_MORE_BUTTON"] && isset($_REQUEST['page'])): ?>
        <script>
            // Указываем, что есть еще страницы для подгрузки
            document.querySelector('.js-load-more').dataset.page = "<?=intval($_REQUEST['page']) + 1?>";

            // Если текущая страница последняя, скрываем кнопку
            if (<?=intval($_REQUEST['page']) + 1?> >= <?=$arResult['NAV_RESULT']->NavPageCount?>) {
                document.querySelector('.js-load-more').style.display = 'none';
            }
        </script>
    <?php endif; ?>
