...
function updateResetButton(filterContainer, activeFilters) {
    ...
    if (activeFilters.length > 0) {
        ...
        if (resetButton) {
            ...
            resetButton.className = 'tag tag--filter tag--reset';
            resetButton.setAttribute('href', 'javascript:void(0);');
            resetButton.setAttribute('data-action', 'reset');
            resetButton.innerHTML = '<span>Сбросить фильтры</span>';
...
function loadFilteredHotels(types, containerId) {
    ...
    fetch(finalUrl)
        ...
        .then(html => {
            ...
            // НОВОЕ: Ищем данные пагинации и обновляем кнопку
...
// НОВАЯ функция для обновления кнопки "Показать еще"
function updateLoadMoreButton(dataElement) {
    ...
    if (!btn) {
        console.log('Кнопка не найдена, ищем по data-атрибуту...');
        btn = document.querySelector('[data-container]');
    }

    if (!btn) {
        console.log('Ищем кнопку в родительском контейнере...');
        const hotelsContainer = document.querySelector('.hotels');
        if (hotelsContainer) {
            btn = hotelsContainer.querySelector('.js-load-more');
        }
    }
    ...
    if (btn) {
        ...
        if (showButton) {
            ...
            // ПРАВИЛЬНЫЙ СПОСОБ: сохраняем структуру кнопки
            let newText = 'Показать больше';
            if (nextCount > 0) {
                newText = `Показать больше (${nextCount} из ${remaining})`;
            }
